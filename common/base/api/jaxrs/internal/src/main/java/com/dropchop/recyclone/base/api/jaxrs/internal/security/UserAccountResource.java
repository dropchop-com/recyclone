 ...
 */
@Path(Paths.Security.USER + Paths.Security.USER_ACCOUNTS_SEGMENT)
@DynamicExecContext(value = UserAccountParams.class, internal = true)
@RequiresPermissions(Domains.Security.ROLE_NODE + PERM_DELIM + Actions.VIEW)
public interface UserAccountResource<P extends UserAccount> {
  ...
  @Produces(MediaType.APPLICATION_JSON_DROPCHOP_RESULT)
  Result <P> search(UserAccountParams params);

  @POST
  @Path(Paths.SEARCH_SEGMENT)
  @Produces(MediaType.APPLICATION_JSON)
  List <P> searchRest(UserAccountParams params);
