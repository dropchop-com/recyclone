...
import com.dropchop.recyclone.base.api.model.utils.RelaxedJson;
...
import static com.dropchop.recyclone.base.api.model.expr.ReservedSymbols.*;
import static com.dropchop.recyclone.base.api.model.utils.Iso8601.DATE_TIME_MS_TZ_FORMATTER;
...
public class AttributeMarshaller {
  ...
  public static class ParserListener implements RelaxedJson.Listener<AttributeSet, AttributeValueList<Object>> {
    ...
    public void onProperty(AttributeSet container, String name, Object value) {
      ...
      Attribute<?> attribute;
      switch (value) {
        case AttributeSet set -> {
          set.setName(name);
          attribute = set;
        }
        case AttributeValueList<?> attributeValueList -> {
          attributeValueList.setName(name);
          attribute = attributeValueList;
        }
        case String s -> attribute = new AttributeString(name, s);
        case Boolean b -> attribute = new AttributeBool(name, b);
        case BigDecimal bigDecimal -> attribute = new AttributeDecimal(name, bigDecimal);
        case ZonedDateTime zonedDateTime -> attribute = new AttributeDate(name, zonedDateTime);
        case null -> {
        ...
        default -> throw new IllegalArgumentException("Unsupported property [" + name + "] value type []");
      }
